...
class CredentialStore:
    def __init__(self, path="wordlists/credentials.csv"):
        ...
        sshmap_logger.debug(f"Loaded {len(self.credentials)} credentials from {self.path}")


    def _read_all(self):
        sshmap_logger.debug(f"Loading credentials from {self.path}")
        if not os.path.exists(self.path):
            sshmap_logger.warning(f"Credential file {self.path} does not exist. Creating a new one.")
            return []
        credentials = []
